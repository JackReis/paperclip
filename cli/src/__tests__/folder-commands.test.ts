...
describe("folder commands", () => {
  ...
  it("wraps full folder CRUD endpoints", async () => {
    ...
    await run(["folder", "create", "--company-id", COMPANY_ID, "-n", "Engineering", "-p", PARENT_ID]);
    await run(["folder", "get", FOLDER_ID, "--company-id", COMPANY_ID]);
    await run(["folder", "update", FOLDER_ID, "--company-id", COMPANY_ID, "-n", "Eng"]);
    await run(["folder", "delete", FOLDER_ID, "--company-id", COMPANY_ID]);
  ...
  it("folder agents lists agents in a folder", async () => {
    const fetchMock = vi.fn().mockImplementation(() =>
      Promise.resolve(jsonResponse([{ id: "a1", name: "Bot", adapterType: "hermes_local", status: "idle", folderId: null }]))
    );
    vi.stubGlobal("fetch", fetchMock);

    await run(["folder", "agents", FOLDER_ID, "--company-id", COMPANY_ID, "-r"]);
    expect(fetchMock.mock.calls[0][0]).toBe(
      `http://localhost:3100/api/companies/${COMPANY_ID}/agent-folders/${FOLDER_ID}/agents?recursive=true`,
    );
  });
  ...
  it("folder delete --force cascades via recursive deletes", async () => {
    ...
    await run(["folder", "delete", FOLDER_ID, "--company-id", COMPANY_ID, "--force"]);
    // One GET (list with children) + DELETE child + DELETE self = 2 DELETEs
    const deletes = fetchMock.mock.calls
      .filter((c) => c[1]?.method === "DELETE")
      .map((c) => c[0]);
    expect(deletes).toContain(`http://localhost:3100/api/companies/${COMPANY_ID}/agent-folders/${childId}`);
    expect(deletes).toContain(`http://localhost:3100/api/companies/${COMPANY_ID}/agent-folders/${FOLDER_ID}`);
  });
});
