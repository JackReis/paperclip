...
ALTER TABLE "run_events"
  ...
  ADD COLUMN IF NOT EXISTS "is_subscription_included" boolean NOT NULL DEFAULT false;
...
COMMENT ON COLUMN "run_events"."cost_confidence" IS
  ...
  || 'Distinct from generic confidence field.';
...
COMMENT ON COLUMN "run_events"."native_total_tokens" IS
  'Provider native total token count (e.g. OpenAI usage.total_tokens). '
  || 'Kept separate from recomputed estimates.';
COMMENT ON COLUMN "run_events"."recomputed_total_tokens" IS
  'Recomputed total = input + output + cached + reasoning + tool_call tokens.';
